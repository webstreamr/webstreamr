...
describe('FileLions', () => {
  test('filelions f', async () => {
    expect(await extractorRegistry.handle(ctx, new URL('https://filelions.to/f/tyn45apubte2'), CountryCode.en)).toMatchSnapshot();
  });

  test('mivalyo v referer lock', async () => {
    expect(await extractorRegistry.handle(ctx, new URL('https://mivalyo.com/v/tah5znapz3e5'), CountryCode.de, '', 'https://kinoger.com')).toMatchSnapshot();
  });
});
