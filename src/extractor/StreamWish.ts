...
export class StreamWish extends Extractor {
  ...
  public override normalize(url: URL): URL {
    return new URL(
      url.href.replace('/f/', '/e/').replace('/d/', '/e/'),
    );
  }
  ...
  private extractMaster(text: string, base: URL): URL | null {
    for (const pattern of this.masterPatterns) {
  ...
  protected async extractInternal(
    ...
    try {
      const embedHtml = await this.fetcher.text(
        ctx,
        rewritten,
        requestCfg,
      );

      const iframeUrl = this.extractIframe(
        embedHtml,
        rewritten,
      );

      const packedHtml = await this.fetcher.text(
        ctx,
        iframeUrl,
        requestCfg,
      );

      const masterUrl = this.extractMaster(
        packedHtml,
        iframeUrl,
      );
